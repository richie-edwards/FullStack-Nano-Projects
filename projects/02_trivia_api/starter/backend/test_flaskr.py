...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        category = Category.query.filter(
            Category.type == "science").one_or_none()
        self.new_question = {
    ...
    def tearDown(self):
        ...
        question_to_delete = Question.query.filter(
            Question.question == "How many miles away from earth is the moon?")
    ...
    def test_get_paginated_questions(self):
        ...
        self.assertEqual(
            response.status_code,
            200,
            f'Status code {response.status_code} was not expected')
        self.assertTrue(data['success'])
        self.assertTrue(len(data['questions']) <= 10 and
    ...
    def test_delete_question(self):
        question = Question.query.filter(
            Question.question == "What boxer's original "
            "name is Cassius Clay?").one_or_none()
        self.assertIsNotNone(
            question,
            "please make sure the question exists before testing delete")
    ...
    def test_create_question(self):
        ...
        data = json.loads(response.data)
        my_new_question = Question.query.filter(
            Question.question == self.new_question['question']).one_or_none()

        # Assert
        self.assertTrue(
            data['success'],
            "the success result was False")
        self.assertEqual(
            self.new_question['question'],
            my_new_question.question)

    """ Test that we get a 422 (unprocessable entity) when
    trying to create a question with missing answer """

    def test_422_create_question_missing_answer(self):
        response = self.client().post(
            '/questions', json=self.new_question_missing_answer)
        data = json.loads(response.data)

        self.assertFalse(
            data['success'],
            "The success result was True")
    ...
    def test_search_question(self):
        filtered_questions = Question.query.filter(
            Question.question.ilike(f"%{self.search_term['searchTerm']}"
                                    "%")).all()
    ...
    def test_quizzes(self):
        ...
        gogh_questions = Question.query.filter(
            Question.question.ilike('%' + 'gogh' + '%')).all()
        ...
        self.assertTrue(data['success'])
        self.assertTrue(
            str(data['question']['id']) not in gogh_questions)

    """ Test that we get questions by category"""

    def test_questions_by_category(self):
        sports = Category.query.filter(
            Category.type == 'Sports').one_or_none()

        response = self.client().get(
            f'/categories/{sports.id}/questions')
        data = json.loads(response.data)
